...
export default function UserProfile() {
  ...
  return (
    <div className="relative">
      ...
      <button
        className="bg-yellow-500 rounded-full w-11 h-11 flex items-center justify-center"
      ...
      {isPopupOpen && (
        <div ref={popupRef} className="text-center text-primary-800 absolute right-0 min-w-[200px] w-25vw bg-primary-50 rounded-md shadow-xl">
          <div className="text-center p-6 border-b relative">
            <img
              className={`cursor-pointer h-full w-full bg-yellow-500 rounded-full`}
              src={userData.pic}
              alt="Profile"
            />
          </div>
          ...
          {/* logout button */}
          <button onClick={(e) => { handelLogout() }} className="cursor-pointer w-full py-2 px-4 text-sm font-bold hover:bg-green-500 hover:text-primary-50">
            LOGOUT
          </button>
        </div>
      )}
