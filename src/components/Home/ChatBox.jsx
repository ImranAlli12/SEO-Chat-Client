...
export default function ChatBox() {
  ...
  if (!selectedUserForChat) {
    return (
      <div className="flex flex-col h-4/5 mt-8 bg-white rounded-lg px-4 py-2 pb-4">
        <div className="flex flex-col items-center justify-center h-full">
          <img className="w-1000 h-500 mr-2" src={logo} alt="logo" />
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-4/5 bg-green-300 rounded-bl-lg rounded-br-lg px-4 py-2 pb-4">
        <div className="flex h-full flex-col max-h-[75vh] overflow-y-auto bg-white  rounded-lg mb-2">
          ...
          <ScrollableFeed>
            {Array.isArray(getMessageData) && getMessageData.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full">
                ...
                <p className="text-blaack">Start Chating!</p>
              </div>
            ) : (
        ...
        <div className="relative mt-2">
          ...
          <button
            disabled={sendMessageProcessing}
            type="button"
            className="absolute inset-y-0  right-1 top-1 bottom-1 px-2.5 py-1 rounded-lg hover:bg-green-700 bg-green-500 text-primary-100 focus:outline-none"
