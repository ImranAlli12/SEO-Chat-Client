...
const DisplayChatCard = ({ item }) => {
  ...
  return (
    <div onClick={handelSelectUserForChat} className="bg-green-400 cursor-pointer rounded-lg shadow-md mt-3 hover:shadow-lg hover:ring-2 hover:ring-primary-200 hover:bg-green-500 transition-shadow duration-200">
      <div className="flex items-center space-x-4 p-2">
        <div className="flex-shrink-0">
          <img className="w-10 h-10  rounded-full" src={item.isGroupChat ? "icons8-collaborating-in-circle-48.png" : selectedUser.pic} alt={`${item.name} image`} />
        </div>
